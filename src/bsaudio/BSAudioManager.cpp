...
void BSAudioManager::SetConditions(float afCurrentGamehour,
								   uint aiWeatherConditions) {
...
BSSoundMessage* BSAudioManager::GetFreeQueueMessage() {
	return FreeQueueMessages.Pop();
}

void BSAudioManager::FreeQueueMessage(BSSoundMessage* apMsgToFree) {}

void BSAudioManager::ComposeMessage(SOUND_MSG aMsg, uint aiID, int aiData,
									void* apPointerData,
...
void BSAudioManager::StopMovingSounds(NiAVObject* apObj, float afFadeSeconds,
									  uint aiFlags) {}

bool BSAudioManager::SetStaticAttenuation(uint aiSoundID,
										  unsigned short aiStaticAttenutation) {
	return 0;
}

bool BSAudioManager::SetFrequency(uint aiSoundID, float afNewFrequency) {
	return 0;
}

bool BSAudioManager::SetModFrequency(uint aiSoundID, float afNewFrequency) {
	return 0;
}

bool BSAudioManager::SetFrequencyVariance(uint aiSoundID,
										  unsigned char aiFreqVariance) {
	return 0;
}

bool BSAudioManager::SetBalance(uint aiSoundID, int aiNewBalance) { return 0; }

bool BSAudioManager::SetPosition(uint aiSoundID, float afX, float afY,
								 float afZ) {
	return 0;
}

bool BSAudioManager::SetBeamEnd(uint aiSoundID, float afX, float afY,
								float afZ) {
	return 0;
}
...
void BSAudioManager::SetAttenuationCurve(uint aiSoundID, unsigned short aiPt1,
										 unsigned short aiPt2,
										 unsigned short aiPt3,
										 unsigned short aiPt4,
										 unsigned short aiPt5) {}

void BSAudioManager::SetReverbAttenuation(uint aiSoundID,
										  unsigned short aiReverbAtt) {}

bool BSAudioManager::SetOrientation(uint aiSoundID, float afX, float afY,
									float afZ) {
	return 0;
}
...
bool BSAudioManager::SetCompletionCallback(uint aiSoundID,
										   void (*apfnCallback)(void*, bool),
										   void* apSoundContext) {
	return 0;
}

bool BSAudioManager::SetPlayCallback(uint aiSoundID,
									 void (*apfnCallback)(void*, int),
									 void* apSoundContext) {
	return 0;
}
...
void BSAudioManager::SetTimeConstraints(uint aiSoundID, unsigned char aiStart,
										unsigned char aiStop) {}

void BSAudioManager::FadeOut(uint aiID, uint aiMilliseconds,
							 uint aiFollowUpAction) {}

void BSAudioManager::FadeIn(uint aiID, uint aiMilliseconds,
							uint aiFollowUpAction) {}
...
void BSAudioManager::MuteAllOfType(uint aiTargetType, uint aiFadeTimeMS,
								   uint aiMillibels) {}
...
void BSAudioManager::Precache(char const* apFilenameToCache, uint aiFlags,
...
// BSSoundHandle
// BSAudioManager::GetSoundHandleByName(char const *apSoundName, uint
// aiTypeFlags) {}
