...
bool BSSoundHandle::PlayAfter(uint aiDelay, int aiPlayFlags) {
	...
	eState = ASSUMED_STATE_PLAYING;
	return BSAudioManager::QInstance().PlayAfter(iSoundID, aiDelay,
												 aiPlayFlags);
}
...
bool BSSoundHandle::SetModFrequency(float afNewFrequency) {
	...
	return BSAudioManager::QInstance().SetModFrequency(iSoundID,
													   afNewFrequency);
}
...
bool BSSoundHandle::SetFrequencyVariance(u8 aiFreqVariance) {
	...
	return BSAudioManager::QInstance().SetFrequencyVariance(iSoundID,
															aiFreqVariance);
}
...
void BSSoundHandle::SetAttenuationCurve(u16 aiPt1, u16 aiPt2, u16 aiPt3,
										u16 aiPt4, u16 aiPt5) {
	if (iSoundID == -1) {
	} else {
		BSAudioManager::QInstance().SetAttenuationCurve(iSoundID, aiPt1, aiPt2,
														aiPt3, aiPt4, aiPt5);
	}
}
...
void BSSoundHandle::SetTimeConstraints(unsigned char aiStart,
									   unsigned char aiStop) {
	if (iSoundID == -1) {
	} else {
		BSAudioManager::QInstance().SetTimeConstraints(iSoundID, aiStart,
													   aiStop);
	}
}

void BSSoundHandle::SetCompletionCallback(void (*apfnCallback)(void*, bool),
										  void* apContext) {
...
void BSSoundHandle::SetPlayCallback(void (*apfnCallback)(void*, int),
									void* apContext) {
	if (!BSAudio::QInstance().IsAudioEnabled() && apfnCallback != nullptr) {
		apfnCallback(apContext, 5000);
	}
	if (iSoundID != -1) {
		BSAudioManager::QInstance().SetPlayCallback(iSoundID, apfnCallback,
													apContext);
	}
}
