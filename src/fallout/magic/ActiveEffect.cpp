...
void ActiveEffect::SetEnchantmentSource(TESBoundObject *apSource) {
    pSource = apSource;
    if (apSource != nullptr) {
        if (apSource->GetFormType() != ARMO_ID && apSource->GetFormType() != CLOT_ID)
            return;
        iFlags |= 0x100;
    }
}
...
ActiveEffect *ActiveEffect::LoadActiveEffect(BGSLoadFormBuffer *apLoadGameBuffer) {
    return nullptr;
}
...
ActiveEffect *ActiveEffect::Copy() {
    ActiveEffect *new_effect = new (
        "D:\\_Fallout3\\Platforms\\Common\\Code\\Fallout\\Magic\\ActiveEffect.cpp",
        124,
        __FUNCTION__
    ) ActiveEffect(pCaster, pSpell, pEffect);
    CopyData(new_effect);
    return new_effect;
}
