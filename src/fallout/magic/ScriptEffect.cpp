...
ActiveEffect* ScriptEffect::Copy() {
	ScriptEffect* new_effect = new (
		"D:\\_Fallout3\\Platforms\\Common\\Code\\Fallout\\Magic\\ScriptEffect."
		"cpp",
		55, __FUNCTION__) ScriptEffect(pCaster, pSpell, pEffect);
	CopyData(new_effect);
	return new_effect;
}

ActiveEffect* ScriptEffect::Instantiate(MagicCaster* apCaster,
										MagicItem* apSpell,
										EffectItem* apEffect) {
	ScriptEffect* new_effect = new (
		"D:\\_Fallout3\\Platforms\\Common\\Code\\Fallout\\Magic\\ScriptEffect."
		"cpp",
		101, __FUNCTION__) ScriptEffect(apCaster, apSpell, apEffect);
...
void ScriptEffect::Start() {
	if (pScript != nullptr) {
		...
		pScript->RunScriptEffectStart(pTarget->GetTargetStatsObject(),
									  pEffectLocals);
	}
}
...
void ScriptEffect::Finish() {
	if (pScript != nullptr) {
		pScript->RunScriptEffectFinish(pTarget->GetTargetStatsObject(),
									   pEffectLocals);
	}
...
void ScriptEffect::Update(float afSecondsElapsed) {
	if (pScript != nullptr) {
		pScript->RunScriptEffectUpdate(pTarget->GetTargetStatsObject(),
									   pEffectLocals, afSecondsElapsed);
	}
}
