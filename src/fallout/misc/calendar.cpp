...
void Calendar::InitGlobals() {
	...
	pGameDaysPassed =
		static_cast<TESGlobal*>(TESForm::GetFormByNumericID(0x39));
	pTimeScale = static_cast<TESGlobal*>(TESForm::GetFormByNumericID(0x3a));
	pGameDaysPassed->SetValue(pGameHour->GetValue() / 24.f +
							  pGameDaysPassed->GetValue());
	iMidnightsPassed = 0;
}
...
char Calendar::GetDay() {
	return pGameDay != nullptr ? pGameDay->GetValue() : 17.0f;
}

float Calendar::GetHour() {
	return pGameHour != nullptr ? pGameHour->GetValue() : 12.0f;
}
...
uint Calendar::GetHoursPassed() {
	return floorf(
		(pGameDaysPassed != nullptr ? pGameDaysPassed->GetValue() : 1.0f) *
		24.f);
}
