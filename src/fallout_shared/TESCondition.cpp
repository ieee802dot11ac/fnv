...
bool TESCondition::CheckValue(
    ENUM_COMPARISON_CONDITION aiCondition, float afVal1, float afVal2
) {
...
bool TESCondition::IsTrue(
    TESObjectREFR *apActionRef,
...
TESConditionItem *TESCondition::GetConditionItem(int aiIndex) { return nullptr; }

bool TESCondition::IsTrue(TESObjectREFR *apActionRef, TESObjectREFR *apTargetRef) {
    bool b;
...
void TESCondition::Load(TESFile *apFile) {
    if (apFile == nullptr || apFile->GetTESChunk() != CIDA_ID)
        return;
    TESConditionItem *item = new (
        "D:\\_Fallout3\\Platforms\\Common\\Code\\Fallout Shared\\TESCondition.cpp",
        122,
        __FUNCTION__
    ) TESConditionItem;
    item->Load(apFile);
...
void TESCondition::Copy(TESCondition *apCopy, TESForm *apOwner) {
    ...
    for (auto *conds = &apCopy->listConditions; conds != nullptr && !conds->IsEmpty();) {
        TESConditionItem *item = new (
            "D:\\_Fallout3\\Platforms\\Common\\Code\\Fallout Shared\\TESCondition.cpp",
            162,
            __FUNCTION__
