...
void TESDescription::LoadDescriptionData(TESDescription* apDesc,
										 TESFile* apFile) {
	if (apDesc && apFile) {
...
void TESDescription::SaveDescriptionData(TESDescription* apDesc,
										 CHUNK_ID aiChunk) {
	if (apDesc == nullptr)
		return;
}
...
bool TESDescription::CompareComponent(BaseFormComponent* apCompareFC) {
	TESDescription* apCompareFC_Desc =
		dynamic_cast<TESDescription*>(apCompareFC);
	if (apCompareFC_Desc == nullptr) {
		return true;
	}
	// copy cDescription? i guess?
	return (cDescription.StrCmp(apCompareFC_Desc->GetDescription(), false) != 0)
			   ? 0
			   : 1;
}
