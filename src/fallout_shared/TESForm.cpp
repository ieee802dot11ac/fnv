...
void TESForm::SaveGameDataOLD(u16* apData, int aiSize) {}

void TESForm::SaveGameDataOLD(uint* apData, int aiSize) {}
...
void TESForm::SaveGame(BGSSaveFormBuffer* apSaveGameBuffer) {
	if (apSaveGameBuffer->GetChangeFlags().CheckFlags(1)) {
		apSaveGameBuffer->SaveDataEndian(&iFormFlags, sizeof(iFormFlags), 4, 0);
	}
}
...
// void TESForm::CreateGroupData(FORM *apOutGroupFORM, FORM_GROUP
// *apParentGroup) {}
...
void TESForm::__AddChunkData(CHUNK_ID aeChunkID, void const* apData,
							 uint aiSize) {
	BS_ASSERT_NOFUNC(
		(aiSize & 0xFFFF) != 0,
...
void TESForm::StartForm() {
	if (!GetTemporary()) {
		...
		FORM* form = new ("D:\\_Fallout3\\Platforms\\Common\\Code\\Fallout "
						  "Shared\\TESForm.cpp",
						  1878, __FUNCTION__) FORM;
		pFormBuffer = form;
...
void TESForm::AddChunk(CHUNK_ID aeChunkID) {
	__AddChunkData(aeChunkID, nullptr, 0);
}

void TESForm::AddChunkArray(CHUNK_ID aeChunkID, char const* apData,
							uint aiSize) {
	__AddChunkData(aeChunkID, apData, aiSize);
}

void TESForm::AddChunkArray(CHUNK_ID aeChunkID, float const* apData,
							uint aiCount) {}

void TESForm::AddChunkArray(CHUNK_ID aeChunkID, short const* apData,
							uint aiCount) {
	AddChunkArray16(aeChunkID, apData, aiCount);
}

void TESForm::AddChunkArray32(CHUNK_ID aeChunkID, void const* apData,
							  uint aiCount) {}

void TESForm::AddChunkArray16(CHUNK_ID aeChunkID, void const* apData,
							  uint aiCount) {
	aiCount *= 2;
	if (bEndianSwapOnSave) {
	}
...
void TESForm::SaveData() { __SaveData(nullptr, 0); }
