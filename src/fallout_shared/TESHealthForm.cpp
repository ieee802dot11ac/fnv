...
uint TESHealthForm::GetFormHealth(TESForm *apForm) {
    ...
    ENUM_FORM_ID formtype = apForm->GetFormType();
    switch (formtype) {
    // case WEAP_ID: return static_cast<TESObjectWEAP*>(apForm)->GetFormHealth();
    // case ARMO_ID: return static_cast<TESObjectARMO*>(apForm)->GetFormHealth();
    // case NPC__ID:
    // case CREA_ID: return static_cast<TESNPC*>(apForm)->GetFormHealth();
    default:
        return 0;
    }
    return 0;
