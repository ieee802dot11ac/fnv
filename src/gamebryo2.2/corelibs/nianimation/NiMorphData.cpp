...
NiObject* NiMorphData::CreateObject() {
	NiObject* obj = new ("D:\\_Fallout3\\Platforms\\Common\\Code\\Gamebryo2."
						 "2\\CoreLibs\\NiAnimation\\NiMorphData.cpp",
						 179, __FUNCTION__) NiMorphData;
	return obj;
}
...
bool NiMorphData::RegisterStreamables(NiStream& kStream) {
	...
	for (int i = 0; i < m_uiNumTargets; i++) {
		(i < m_uiNumTargets ? &m_aMorphTargets[i] : nullptr)
...
bool NiMorphData::IsEqual(NiObject* pObject) {
	...
	for (int i = 0; i < m_uiNumTargets; i++) {
		...
		if (!(i < m_uiNumTargets ? &m_aMorphTargets[i] : nullptr)
				 ->IsEqual((i < other->m_uiNumTargets
								? &other->m_aMorphTargets[i]
								: nullptr),
