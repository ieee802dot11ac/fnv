...
class NiSkinPartition : public NiObject {
  public:
	...
	class Partition { /* Size=0x2c */
	  public:
		...
		Partition();
		virtual ~Partition();
		void Create(const TriangleSet&, const BoneSet&,
					const NiTriBasedGeomData*, const VertexInteractions*, bool,
					bool, u8, uint);
	  ...
	  protected:
		...
		void CreateTriangleList(const TriangleSet&, const NiTriBasedGeomData*,
								const u16*);
	};
	...
	bool MakePartitions(const NiTriBasedGeomData*, const NiSkinData*, u8, u8,
						bool, int*, u16*, bool*);
  ...
  protected:
	bool MakeBoneSets(const NiTriBasedGeomData*, const NiSkinData*, u8,
					  VertexInteractions*, NiTPrimitiveSet<BoneSet*>&, uint*&,
					  const int*);
	VertexInteractions* CreateVertexInteractions(uint, const NiSkinData*);
	bool RemoveExcessInteractions(const NiTriBasedGeomData*, u8, u8,
								  VertexInteractions*);
	void CreateInitialBoneSets(const NiTriBasedGeomData*, VertexInteractions*,
							   NiTPrimitiveSet<BoneSet*>&, const int*);
	void MergeBoneSets(u8, NiTPrimitiveSet<BoneSet*>&);
	void MakeBoneMap(NiTPointerMap<uint, float>&, const u16*,
					 const VertexInteractions*);
	void MergeIntoPartition(Partition&, Partition*, uint, uint, u8, u8, u16*,
							uint);

	uint m_uiPartitions;	   // 0x8
	Partition* m_pkPartitions; // 0xc

	static int AssignTriangle(NiTPrimitiveSet<BoneSet*>&, const u16*,
							  VertexInteractions*, bool*, bool);
};
