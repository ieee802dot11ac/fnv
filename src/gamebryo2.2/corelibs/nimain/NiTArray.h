...
template <typename T> class NiTPrimitiveArray : public NiTArray<T> {
  public:
...
template <typename T, typename Alloc = NiTMallocInterface<T> >
class NiTLargeArray {
  public:
	NiTLargeArray(uint, uint);
  ...
  protected:
	T* m_pBase;		  // 0x04
	uint m_uiMaxSize; // 0x08
	uint m_uiSize;	  // 0x0c
	uint m_uiESize;	  // 0x10
	uint m_uiGrowBy;  // 0x14
};

template <typename T> class NiTLargePrimitiveArray : public NiTLargeArray<T> {
  public:
	NiTLargePrimitiveArray(uint, uint);
