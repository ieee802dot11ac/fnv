...
#define VERSION(hh, hl, lh, ll)                                                \
	((u8(hh) << 24) | (u8(hl) << 16) | (u8(lh) << 8) | u8(ll))
...
template <typename T> void NiStreamLoadBinary(NiStream&, T&);

template <typename T> void NiStreamSaveBinary(NiStream&, const T&);
