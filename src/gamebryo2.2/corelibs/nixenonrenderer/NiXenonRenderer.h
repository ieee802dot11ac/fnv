...
public:
    ...
    virtual void
    SetDefaultProgramCache(NiFragmentMaterial *, bool, bool, bool, bool, const char *);
    void SetDefaultProgramCache(NiFragmentMaterial *);
    void PurgeAllRendererData(NiAVObject *);
    bool
    MapWindowPointToBufferPoint(uint, uint, float &, float &, const NiRenderTargetGroup *);
    virtual const NiPixelFormat *
    FindClosestDepthStencilFormat(const NiPixelFormat *, uint, uint) const;
    const NiPixelFormat *FindClosestDepthStencilFormat(const NiPixelFormat *) const;
    ...
    NiXenonRenderer::RecreateStatus Recreate(
        uint,
        uint,
        uint,
        D3DFORMAT,
    ...
    virtual const NiPixelFormat *FindClosestPixelFormat(NiTexture::FormatPrefs &) const;
    virtual const NiPixelFormat *
        FindClosestPixelFormat(NiXenonRenderer::TexFormat, NiXenonRenderer::TexUsage);
    ...
    virtual bool PrecacheGeometry(NiGeometry *, uint, uint, NiD3DShaderDeclaration *);
    ...
    virtual NiPixelData *
    TakeScreenShot(const NiRect<uint> *, const NiRenderTargetGroup *);
    virtual bool
    FastCopy(const Ni2DBuffer *, Ni2DBuffer *, const NiRect<uint> *, uint, uint);
    virtual bool Copy(
        const Ni2DBuffer *,
        Ni2DBuffer *,
        const NiRect<uint> *,
        const NiRect<uint> *,
        Ni2DBuffer::CopyFilterPreference
    );
    ...
    void SetSkinnedModelTransforms(
        const NiSkinInstance *, const NiSkinPartition::Partition *, const NiTransform &
    );
    void
    CalculateBoneMatrices(const NiSkinInstance *, const NiTransform &, bool, uint, bool);
    void DrawPrimitive(
        NiGeometry *,
        NiGeometryData *,
        NiSkinInstance *,
    ...
    void
    FreeGeometricData(NiGeometryData *, NiSkinInstance *, NiSkinPartition::Partition *);
    virtual bool CreateSourceTextureRendererData(NiSourceTexture *);
    virtual bool CreateRenderedTextureRendererData(
        NiRenderedTexture *, Ni2DBuffer::MultiSamplePreference
    );
    ...
    virtual bool CreateDepthStencilRendererData(
        NiDepthStencilBuffer *, const NiPixelFormat *, Ni2DBuffer::MultiSamplePreference
    );
    ...
    const NiPixelFormat *GetDefaultTextureFormat(NiXenonRenderer::TexUsage) const;
    ...
    bool PackSkinnedGeometryBuffer(
        NiGeometryBufferData *,
        NiGeometryData *,
        const NiSkinInstance *,
    ...
    virtual void Do_DrawPartition(
        NiGeometryData::RendererData *, NiSkinPartition::Partition *, NiGeometryData *
    );
    NiXenonRenderer(const NiXenonRenderer &);
    bool IsHardwareSkinned(NiGeometry *, const NiD3DShaderInterface *);
    NiXenonRenderTargetGroupData *GetRenderTargetGroupData(NiRenderTargetGroup *) const;
...
protected:
    ...
    void *PackStripParticleVertex(
        NiXenonRenderer::SourceNormalVertex *,
        const NiPoint3 &,
        const NiPoint3 &,
        uint,
        u16,
    ...
    void *PackStripParticleVertex(
        NiXenonRenderer::SourceNormalVertex *, const NiPoint3 &, int, uint, u16, u16, float
    );
    NiXenonRenderer();
    void InitToDefault();
    bool LostDeviceRestore();
    bool InitializePresentParams(
        uint,
        uint,
        uint,
        uint,
    ...
    uint
    PackParticleStrips(NiXenonRenderer::SourceNormalVertex *, u16 *, NiParticlesData *);
...
protected:
    ...
    NiTPointerMap<NiVBBlock *, NiXenonRenderer::PrePackObject *> m_kPrePackObjects; // 0x424
