...
CIndexedGeometry::CIndexedGeometry(CWindEngine* pWindEngine,
								   bool bRetainTexCoords)
	: m_bRetainTexCoords(bRetainTexCoords), m_pWindEngine(pWindEngine),
	  m_bVertexWeighting(false), m_bManualLighting(false),
	  m_pVertexWindComputed(nullptr), m_nVertexSize(0), m_bValid(1),
	  m_eWindMethod(CSpeedTreeRT::WIND_NONE), m_nNumLodLevels(0),
	  m_nVertexCounter(0), m_nVertexCounterLodLevel(0), m_nStripCounter(0) {
		  ADDSTACK(0x80)
	  };
...
void CIndexedGeometry::AddVertexTexCoord0(const float* pTexCoord,
										  short sIndex) {}
...
void CIndexedGeometry::ChangeTexCoord(u8 chChangedMapIndex,
									  float* pNewTexCoords) {}
