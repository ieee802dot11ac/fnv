...
namespace D3DX {
    ...
    */
     ...
     */
    void png_reset_crc(png_structp png_ptr) { png_ptr->crc = crc32(0, Z_NULL, 0); }
    ...
    png_infop png_create_info_struct(png_structp png_ptr) {
        ...
        if ((info_ptr =
                 (png_infop)png_create_struct_2(PNG_STRUCT_INFO, png_ptr->malloc_fn))
            != NULL)
#else
    ...
    png_charp png_convert_to_rfc1123(png_structp png_ptr, png_timep ptime) {
        static PNG_CONST char short_months[12][4] = { "Jan", "Feb", "Mar", "Apr",
                                                      "May", "Jun", "Jul", "Aug",
                                                      "Sep", "Oct", "Nov", "Dec" };

        if (png_ptr->time_buffer == NULL) {
            png_ptr->time_buffer =
                (png_charp)png_malloc(png_ptr, (png_uint_32)(29 * sizeof(char)));
        }
...
#ifdef USE_FAR_KEYWORD
        {
            ...
            png_memcpy(png_ptr->time_buffer, near_time_buf, 29 * sizeof(char));
        }
#else
        sprintf(
            png_ptr->time_buffer,
   ...
   Copyright (c) 1998, 1999 Glenn Randers-Pehrson\n");
    ...
    void png_check_version(version_1_0_5 png_h_is_not_version_1_0_5) {
