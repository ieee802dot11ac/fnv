...
namespace D3DX {
    ...
    png_structp png_create_read_struct(
        ...
        return (png_create_read_struct_2(
            user_png_ver, error_ptr, error_fn, warn_fn, NULL, NULL, NULL
        ));
    }
    ...
    png_structp png_create_read_struct_2(
...
#ifdef PNG_USER_MEM_SUPPORTED
        if ((png_ptr = (png_structp)
                 png_create_struct_2(PNG_STRUCT_PNG, (png_malloc_ptr)malloc_fn))
            == NULL)
#else
        ...
        png_ptr->zbuf = (png_bytep)png_malloc(png_ptr, (png_uint_32)png_ptr->zbuf_size);
    ...
    void png_read_info(png_structp png_ptr, png_infop info_ptr) {
        ...
        if (png_ptr->sig_bytes < 8) {
            png_size_t num_checked = png_ptr->sig_bytes, num_to_check = 8 - num_checked;
    ...
    void png_read_row(png_structp png_ptr, png_bytep row, png_bytep dsp_row) {
        int ret;
        png_debug2(
            1, "in png_read_row (row %d, pass %d)\n", png_ptr->row_number, png_ptr->pass
        );
        /* save jump buffer and error functions */
        if (!(png_ptr->flags & PNG_FLAG_ROW_INIT))
            png_read_start_row(png_ptr);
        ...
        if (png_ptr->interlaced && (png_ptr->transformations & PNG_INTERLACE)) {
            ...
            case 0:
                if (png_ptr->row_number & 7) {
                    if (dsp_row != NULL)
                        png_combine_row(
                            png_ptr, dsp_row, png_pass_dsp_mask[png_ptr->pass]
                        );
        ...
        do {
            ...
            if (ret != Z_OK)
                png_error(
                    png_ptr,
                    png_ptr->zstream.msg ? png_ptr->zstream.msg : "Decompression error"
                );
        ...
        png_ptr->row_info.rowbytes =
            ((png_ptr->row_info.width * (png_uint_32)png_ptr->row_info.pixel_depth + 7)
             >> 3);
        ...
        png_memcpy_check(
            png_ptr, png_ptr->prev_row, png_ptr->row_buf, png_ptr->rowbytes + 1
        );
    ...
    void png_read_image(png_structp png_ptr, png_bytepp image) {
        ...
        if (png_ptr->interlaced)
            png_error(
                png_ptr, "Cannot read interlaced image -- interlace handler disabled."
            );
        pass = 1;
#endif
