...
namespace D3DX {
    ...
    void png_set_dither(
        ...
        if (!full_dither) {
            ...
            png_ptr->dither_index = (png_bytep)png_malloc(
                png_ptr, (png_uint_32)(num_palette * sizeof(png_byte))
            );
        ...
        if (num_palette > maximum_colors) {
            if (histogram != NULL) {
                ...
                index_to_palette = (png_bytep)png_malloc(
                    png_ptr, (png_uint_32)(num_palette * sizeof(png_byte))
                );
                palette_to_index = (png_bytep)png_malloc(
                    png_ptr, (png_uint_32)(num_palette * sizeof(png_byte))
                );
                ...
                hash = (png_dsortpp)png_malloc(
                    png_ptr, (png_uint_32)(769 * sizeof(png_dsortp))
                );
                for (i = 0; i < 769; i++)
                    hash[i] = NULL;
                    ...
                    for (i = 0; i < num_new_palette - 1; i++) {
                        ...
                        for (j = i + 1; j < num_new_palette; j++) {
                            ...
                            if (d <= max_d) {
                                ...
                                t = (png_dsortp)png_malloc(
                                    png_ptr, (png_uint_32)(sizeof(png_dsort))
                                );
                                t->next = hash[d];
                                t->left = (png_byte)i;
                                t->right = (png_byte)j;
        ...
        if (full_dither) {
            ...
            png_bytep distance;
            int total_bits =
                PNG_DITHER_RED_BITS + PNG_DITHER_GREEN_BITS + PNG_DITHER_BLUE_BITS;
            int num_red = (1 << PNG_DITHER_RED_BITS);
            int num_green = (1 << PNG_DITHER_GREEN_BITS);
            int num_blue = (1 << PNG_DITHER_BLUE_BITS);
            png_size_t num_entries = ((png_size_t)1 << total_bits);

            png_ptr->palette_lookup = (png_bytep)png_malloc(
                png_ptr, (png_uint_32)(num_entries * sizeof(png_byte))
            );

            png_memset(png_ptr->palette_lookup, 0, num_entries * sizeof(png_byte));

            distance = (png_bytep)png_malloc(
                png_ptr, (png_uint_32)(num_entries * sizeof(png_byte))
            );
                ...
                if (png_ptr->transformations & PNG_BACKGROUND) {
                    if (color_type == PNG_COLOR_TYPE_PALETTE) {
                        ...
                        if (png_ptr->background_gamma_type == PNG_BACKGROUND_GAMMA_FILE) {
                            ...
                            case PNG_BACKGROUND_GAMMA_UNIQUE:
                                ...
                                gs = 1.0
                                    / (png_ptr->background_gamma * png_ptr->screen_gamma);
                                break;
                            default:
                                g = 1.0; /* back_1 */
                                gs = 1.0; /* back */
                            ...
                            if (fabs(gs - 1.0) < PNG_GAMMA_THRESHOLD) {
                                ...
                                back.red =
                                    (png_byte)(pow((double)png_ptr->background.red / 255,
                                                   gs)
                                                   * 255.0
                                               + .5);
                                back.green =
                                    (png_byte)(pow((double)png_ptr->background.green
                                                       / 255,
                                                   gs)
                                                   * 255.0
                                               + .5);
                                back.blue =
                                    (png_byte)(pow((double)png_ptr->background.blue / 255,
                                                   gs)
                                                   * 255.0
                                               + .5);
                            }
                    ...
                    /* color_type != PNG_COLOR_TYPE_PALETTE */
                    {
                        ...
                        case PNG_BACKGROUND_GAMMA_UNIQUE:
                            g = 1.0 / (png_ptr->background_gamma);
                            gs =
                                1.0 / (png_ptr->background_gamma * png_ptr->screen_gamma);
                            break;
                        }
                        ...
                        if (color_type & PNG_COLOR_MASK_COLOR) {
                            ...
                            png_ptr->background_1.red =
                                (png_uint_16)(pow((double)png_ptr->background.red / m, g)
                                                  * m
                                              + .5);
                            png_ptr->background_1.green =
                                (png_uint_16)(pow((double)png_ptr->background.green / m,
                                                  g)
                                                  * m
                                              + .5);
                            png_ptr->background_1.blue =
                                (png_uint_16)(pow((double)png_ptr->background.blue / m, g)
                                                  * m
                                              + .5);
                            png_ptr->background.red =
                                (png_uint_16)(pow((double)png_ptr->background.red / m, gs)
                                                  * m
                                              + .5);
                            png_ptr->background.green =
                                (png_uint_16)(pow((double)png_ptr->background.green / m,
                                              ...
                                              + .5);
                            png_ptr->background.blue =
                                (png_uint_16)(pow((double)png_ptr->background.blue / m,
                                                  gs)
                                                  * m
                                              + .5);
                        } else {
                            ...
                            png_ptr->background_1.gray =
                                (png_uint_16)(pow((double)png_ptr->background.gray / m, g)
                                                  * m
                                              + .5);
                            png_ptr->background.gray =
                                (png_uint_16)(pow((double)png_ptr->background.gray / m,
                                                  gs)
                                                  * m
                                              + .5);
                        }
                ...
                if (png_ptr->transformations & PNG_BACKGROUND
                    ...
                    for (i = 0; i < istop; i++) {
                        if (png_ptr->trans[i] == 0) {
                            ...
                            png_composite(
                                palette[i].red, palette[i].red, png_ptr->trans[i], back.red
                            );
                            png_composite(
                                palette[i].green,
                                palette[i].green,
    ...
    void png_do_read_transformations(png_structp png_ptr) {
        ...
        if (png_ptr->transformations & PNG_EXPAND) {
            if (png_ptr->row_info.color_type == PNG_COLOR_TYPE_PALETTE) {
                ...
                if (png_ptr->num_trans)
                    ...
                    png_do_expand(&(png_ptr->row_info), png_ptr->row_buf + 1, NULL);
            }
        }
#endif
        ...
        if (png_ptr->transformations & PNG_STRIP_ALPHA)
            png_do_strip_filler(
                &(png_ptr->row_info), png_ptr->row_buf + 1, PNG_FLAG_FILLER_AFTER
            );
#endif
...
#if defined(PNG_READ_SHIFT_SUPPORTED)
        if (png_ptr->transformations & PNG_SHIFT)
            png_do_unshift(&(png_ptr->row_info), png_ptr->row_buf + 1, &(png_ptr->shift));
#endif
        ...
        if (png_ptr->transformations & PNG_USER_TRANSFORM) {
            ...
            png_ptr->row_info.pixel_depth =
                (png_byte)(png_ptr->row_info.bit_depth * png_ptr->row_info.channels);
            png_ptr->row_info.rowbytes =
                (png_ptr->row_info.width * png_ptr->row_info.pixel_depth + 7) >> 3;
        }
#endif
    }
       ...
       row_info->color_type == PNG_COLOR_TYPE_GRAY)
   {
    ...
    void png_do_gray_to_rgb(png_row_infop row_info, png_bytep row) {
        ...
        if (row_info->bit_depth >= 8 &&
            ...
            row_info->color_type |= PNG_COLOR_MASK_COLOR;
            row_info->pixel_depth = (png_byte)(row_info->channels * row_info->bit_depth);
...
#endif
                    {
                        ...
                        for (i = 0; i < row_width; i++) {
                            ...
                            if (red != green || red != blue) {
                                rgb_error |= 1;
                                *(dp++) =
                                    (png_byte)((rc * red + gc * green + bc * blue) >> 8);
                            } else
                                *(dp++) = *(sp - 1);
                        }
                    }
                    ...
                    if (png_ptr->gamma_16_to_1 != NULL
                        ...
                        for (i = 0; i < row_width; i++) {
                            ...
                            if (red == green && red == blue)
                                ...
                                png_uint_16 red_1 =
                                    png_ptr->gamma_16_to_1
                                        [(red & 0xff) >> png_ptr->gamma_shift][red >> 8];
                                png_uint_16 green_1 =
                                    png_ptr->gamma_16_to_1
                                        [(green & 0xff) >> png_ptr->gamma_shift]
                                        [green >> 8];
                                png_uint_16 blue_1 =
                                    png_ptr->gamma_16_to_1
                                        [(blue & 0xff) >> png_ptr->gamma_shift][blue >> 8];
                                png_uint_16 gray16 =
                                    (png_uint_16)((rc * red_1 + gc * green_1
                                                   + bc * blue_1)
                                                  >> 8);
                                w = png_ptr->gamma_16_from_1
                                        [(gray16 & 0xff) >> png_ptr->gamma_shift]
                                        [gray16 >> 8];
                                rgb_error |= 1;
                            }
...
#endif
                    {
                        ...
                        for (i = 0; i < row_width; i++) {
                            ...
                            if (red != green || red != blue)
                                rgb_error |= 1;
                            gray16 =
                                (png_uint_16)((rc * red + gc * green + bc * blue) >> 8);
...
#endif
                    {
                        ...
                        for (i = 0; i < row_width; i++) {
                            ...
                            if (red != green || red != blue)
                                rgb_error |= 1;
                            *(dp++) =
                                (png_byte)((gc * red + gc * green + bc * blue) >> 8);
                            *(dp++) = *(sp++); /* alpha */
                        }
                    }
        ...
        if (png_ptr->transformations & (PNG_GAMMA | PNG_BACKGROUND)) {
            ...
            if (png_ptr->background_gamma_type == PNG_BACKGROUND_GAMMA_FILE) {
                ...
                if (png_ptr->background_gamma_type == PNG_BACKGROUND_GAMMA_SCREEN
                    ...
                    back.red =
                        (png_byte)(pow((double)png_ptr->background.red / 255, g) * 255.0
                                   + 0.5);
                    back.green =
                        (png_byte)(pow((double)png_ptr->background.green / 255, g) * 255.0
                                   + 0.5);
                    back.blue =
                        (png_byte)(pow((double)png_ptr->background.blue / 255, g) * 255.0
                                   + 0.5);
                }
        ...
        if (background != NULL &&
            ...
            case PNG_COLOR_TYPE_GRAY: {
                ...
                case 1: {
                    ...
                    shift = 7;
                    for (i = 0; i < row_width; i++) {
                        if ((png_uint_16)((*sp >> shift) & 0x1) == trans_values->gray) {
                        ...
                        for (i = 0; i < row_width; i++) {
                            if ((png_uint_16)((*sp >> shift) & 0xf)
                                ...
                                *sp |= (png_byte)(background->gray << shift);
                                ...
                                png_byte g =
                                    (png_byte)((gamma_table[p | (p << 4)] >> 4) & 0xf);
                                *sp &= (png_byte)((0xf0f >> (4 - shift)) & 0xff);
                                *sp |= (png_byte)(g << shift);
                            }
                            if (!shift) {
                ...
                } else /* if (row_info->bit_depth == 16) */
                {
                    ...
                    if (gamma_16 != NULL && gamma_16_from_1 != NULL
                        ...
                        for (i = 0; i < row_width; i++, sp += 8, dp += 6) {
                            png_uint_16 a = (png_uint_16)(((png_uint_16)(*(sp + 6)) << 8)
                                                          + (png_uint_16)(*(sp + 7)));
                            if (a == (png_uint_16)0xffff) {
                                png_uint_16 v;
                        ...
                        for (i = 0; i < row_width; i++, sp += 8, dp += 6) {
                            ...
                            if (a == (png_uint_16)0xffff) {
                                ...
                                *(dp + 5) = (png_byte)(background->blue & 0xff);
                                ...
                                png_uint_16 r = (png_uint_16)(((*sp) << 8) + *(sp + 1));
                                png_uint_16 g =
                                    (png_uint_16)(((*(sp + 2)) << 8) + *(sp + 3));
                                png_uint_16 b =
                                    (png_uint_16)(((*(sp + 4)) << 8) + *(sp + 5));
            ...
            if (row_info->color_type & PNG_COLOR_MASK_ALPHA) {
                ...
                row_info->pixel_depth =
                    (png_byte)(row_info->channels * row_info->bit_depth);
                row_info->rowbytes = ((row_width * row_info->pixel_depth + 7) >> 3);
            }
        }
    }
                ...
                if (row_info->bit_depth == 2) {
                    ...
                    for (i = 0; i < row_width; i += 4) {
                        ...
                        *sp =
                            (png_byte)(((((
                                            int
                                        )gamma_table[a | (a >> 2) | (a >> 4) | (a >> 6)]))
                                        & 0xc0)
                                       | ((((
                                               int
                                           )gamma_table[(b << 2) | b | (b >> 2) | (b >> 4)])
                                           >> 2)
                                          & 0x30)
                                       | ((((
                                               int
                                           )gamma_table[(c << 4) | (c << 2) | c | (c >> 2)])
                                           >> 4)
                                          & 0x0c)
                                       | ((((
                                               int
                                           )gamma_table[(d << 6) | (d << 4) | (d << 2) | d])
                                           >> 6)));
                        sp++;
                    }
                }
            ...
            if (row_info->color_type == PNG_COLOR_TYPE_GRAY) {
                ...
                if (trans_value != NULL) {
                    if (row_info->bit_depth == 8) {
                        ...
                        for (i = 0; i < row_width; i++) {
                            if (((png_uint_16) * (sp) | ((png_uint_16) * (sp - 1) << 8))
                                == gray) {
                                *dp-- = 0;
                                *dp-- = 0;
                            } else {
                    ...
                    row_info->rowbytes = ((row_width * row_info->pixel_depth) >> 3);
                }
                ...
                if (row_info->bit_depth == 8) {
                    ...
                    dp = row + (png_size_t)(row_width << 3) - 1;
                    for (i = 0; i < row_width; i++) {
                        if ((((png_uint_16) * (sp - 4) | ((png_uint_16) * (sp - 5) << 8))
                             == trans_value->red)
                            && (((png_uint_16) * (sp - 2)
                                 | ((png_uint_16) * (sp - 3) << 8))
                                == trans_value->green)
    ...
    void png_do_dither(
...
#endif
        {
            if (row_info->color_type == PNG_COLOR_TYPE_RGB && palette_lookup
                ...
                row_info->pixel_depth = row_info->bit_depth;
                row_info->rowbytes = ((row_width * row_info->pixel_depth + 7) >> 3);
            } else if (row_info->color_type == PNG_COLOR_TYPE_PALETTE && dither_lookup
                       && row_info->bit_depth == 8) {
                sp = row;
                for (i = 0; i < row_width; i++, sp++) {
                    *sp = dither_lookup[*sp];
...
#if defined(PNG_READ_GAMMA_SUPPORTED)
    static int png_gamma_shift[] = { 0x10, 0x21, 0x42, 0x84, 0x110, 0x248, 0x550, 0xff0 };
    ...
    void png_build_gamma_table(png_structp png_ptr) {
        ...
        if (png_ptr->gamma != 0.0) {
            if (png_ptr->bit_depth <= 8) {
                ...
                for (i = 0; i < 256; i++) {
                    png_ptr->gamma_table[i] =
                        (png_byte)(pow((double)i / 255.0, g) * 255.0 + .5);
                }
                ...
                if (png_ptr->transformations & (PNG_BACKGROUND | PNG_RGB_TO_GRAY)) {
                    ...
                    png_ptr->gamma_to_1 =
                        (png_bytep)png_malloc(png_ptr, (png_uint_32)256);

                    for (i = 0; i < 256; i++) {
                        png_ptr->gamma_to_1[i] =
                            (png_byte)(pow((double)i / 255.0, g) * 255.0 + .5);
                    }
                ...
                png_ptr->gamma_16_table = (png_uint_16pp)png_malloc(
                    png_ptr, (png_uint_32)(num * sizeof(png_uint_16p))
                );
                ...
                if (png_ptr->transformations & (PNG_16_TO_8 | PNG_BACKGROUND)) {
                    ...
                    for (i = 0; i < num; i++) {
                        png_ptr->gamma_16_table[i] = (png_uint_16p)png_malloc(
                            png_ptr, (png_uint_32)(256 * sizeof(png_uint_16))
                        );
                    }
                    ...
                    for (i = 0; i < num; i++) {
                        ...
                        ig =
                            (((png_uint_32)i * (png_uint_32)png_gamma_shift[shift]) >> 4);
                        for (j = 0; j < 256; j++) {
                ...
                if (png_ptr->transformations & (PNG_BACKGROUND | PNG_RGB_TO_GRAY)) {
                    ...
                    png_ptr->gamma_16_to_1 = (png_uint_16pp)png_malloc(
                        png_ptr, (png_uint_32)(num * sizeof(png_uint_16p))
                    );

                    for (i = 0; i < num; i++) {
                        png_ptr->gamma_16_to_1[i] = (png_uint_16p)png_malloc(
                            png_ptr, (png_uint_32)(256 * sizeof(png_uint_16))
                        );
                    ...
                    png_ptr->gamma_16_from_1 = (png_uint_16pp)png_malloc(
                        png_ptr, (png_uint_32)(num * sizeof(png_uint_16p))
                    );

                    for (i = 0; i < num; i++) {
                        png_ptr->gamma_16_from_1[i] = (png_uint_16p)png_malloc(
                            png_ptr, (png_uint_32)(256 * sizeof(png_uint_16))
                        );
