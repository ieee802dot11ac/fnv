...
namespace D3DX {
    ...
    png_uint_32 png_get_uint_32(png_bytep buf) {
        png_uint_32 i = ((png_uint_32)(*buf) << 24) + ((png_uint_32)(*(buf + 1)) << 16)
    ...
    png_int_32 png_get_int_32(png_bytep buf) {
        png_int_32 i = ((png_int_32)(*buf) << 24) + ((png_int_32)(*(buf + 1)) << 16)
            + ((png_int_32)(*(buf + 2)) << 8) + (png_int_32)(*(buf + 3));
    ...
    png_uint_16 png_get_uint_16(png_bytep buf) {
        png_uint_16 i =
            (png_uint_16)(((png_uint_16)(*buf) << 8) + (png_uint_16)(*(buf + 1)));
    ...
    void png_handle_IHDR(png_structp png_ptr, png_infop info_ptr, png_uint_32 length) {
        ...
        if (width == 0 || width > PNG_MAX_UINT || height == 0 || height > PNG_MAX_UINT)
        ...
        png_ptr->rowbytes =
            ((png_ptr->width * (png_uint_32)png_ptr->pixel_depth + 7) >> 3);
    ...
    void png_handle_gAMA(png_structp png_ptr, png_infop info_ptr, png_uint_32 length) {
        ...
        if (!(png_ptr->mode & PNG_HAVE_IHDR))
            ...
            info_ptr != NULL && info_ptr->valid & PNG_INFO_gAMA
        ...
        if (info_ptr->valid & PNG_INFO_sRGB)
            if (fabs((float)igamma - 45455.) > 500.) {
                png_warning(
                    png_ptr, "Ignoring incorrect gAMA value when sRGB is also present"
                );
    ...
    void png_handle_cHRM(png_structp png_ptr, png_infop info_ptr, png_uint_32 length) {
        ...
        if (!(png_ptr->mode & PNG_HAVE_IHDR))
            ...
            info_ptr != NULL && info_ptr->valid & PNG_INFO_cHRM
#if defined(PNG_READ_sRGB_SUPPORTED)
            && !(info_ptr->valid & PNG_INFO_sRGB)
#endif
        ) {
        ...
        if (red_x < 0 || red_x > 0.8 || red_y < 0 || red_y > 0.8 || red_x + red_y > 1.0) {
        ...
        if (info_ptr->valid & PNG_INFO_sRGB) {
            if (fabs(white_x - (float).3127) > (float).001
                ...
                png_warning(
                    png_ptr, "Ignoring incorrect cHRM value when sRGB is also present"
                );
#ifndef PNG_NO_CONSOLE_IO
                fprintf(
    ...
    void png_handle_tRNS(png_structp png_ptr, png_infop info_ptr, png_uint_32 length) {
        ...
        png_set_tRNS(
            png_ptr, info_ptr, png_ptr->trans, png_ptr->num_trans, &(png_ptr->trans_values)
        );
    }
#endif
    ...
    void png_handle_hIST(png_structp png_ptr, png_infop info_ptr, png_uint_32 length) {
        ...
        num = (int)length / 2;
        png_ptr->hist =
            (png_uint_16p)png_malloc(png_ptr, (png_uint_32)(num * sizeof(png_uint_16)));
    ...
    void png_handle_pCAL(png_structp png_ptr, png_infop info_ptr, png_uint_32 length) {
        ...
        params =
            (png_charpp)png_malloc(png_ptr, (png_uint_32)(nparams * sizeof(png_charp)));
            ...
            while (png_ptr->zstream.avail_in) {
                ...
                if (!png_ptr->zstream.avail_out || ret == Z_STREAM_END) {
                    if (text == NULL) {
                        ...
                        png_memcpy(text, key, key_size);
                        text_size =
                            key_size + png_ptr->zbuf_size - png_ptr->zstream.avail_out;
                        *(text + text_size) = 0x00;
                    } else {
                        png_charp tmp;
                        ...
                        text = (png_charp)png_malloc(
                            png_ptr,
                            (png_uint_32)(text_size + png_ptr->zbuf_size
...
#define isnonalpha(c) ((c) < 41 || (c) > 122 || ((c) > 90 && (c) < 97))
    ...
    png_do_read_interlace
        ...
        if (row != NULL && row_info != NULL) {
            ...
            row_info->rowbytes =
                ((final_width * (png_uint_32)row_info->pixel_depth + 7) >> 3);
        }
    }
#endif
    ...
    void png_read_finish_row(png_structp png_ptr) {
        ...
        if (png_ptr->interlaced) {
            ...
            do {
                ...
                png_ptr->irowbytes =
                    ((png_ptr->iwidth * (png_uint_32)png_ptr->pixel_depth + 7) >> 3) + 1;

                if (!(png_ptr->transformations & PNG_INTERLACE)) {
                    png_ptr->num_rows = (png_ptr->height + png_pass_yinc[png_ptr->pass]
        ...
        if (!(png_ptr->flags & PNG_FLAG_ZLIB_FINISHED)) {
            ...
            for (;;) {
                ...
                if (ret != Z_OK)
                    png_error(
                        png_ptr,
                        png_ptr->zstream.msg ? png_ptr->zstream.msg
                                             : "Decompression Error"
                    );
    ...
    void png_read_start_row(png_structp png_ptr) {
        ...
        png_ptr->color_type == PNG_COLOR_TYPE_GRAY_ALPHA)
      {
        ...
        /* calculate the maximum bytes needed, adding a byte and a pixel
           for safety's sake */
        row_bytes = ((row_bytes * (png_uint_32)max_pixel_depth + 7) >> 3) + 1
            + ((max_pixel_depth + 7) >> 3);
#ifdef PNG_MAX_MALLOC_64K
...
#endif
        png_ptr->prev_row =
            (png_bytep)png_malloc(png_ptr, (png_uint_32)(png_ptr->rowbytes + 1));
